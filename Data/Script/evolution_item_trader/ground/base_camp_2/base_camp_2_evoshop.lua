...
function base_camp_2_evoshop.Interact_QuestGiver(obj, activator)
	...
	if SV.ModData_EvolutionItemTrader.Quest_Given == false then
		...
		UI:WaitShowDialogue("The path's blocked off by water,[pause=10] but we'll help you across.")
...
function base_camp_2_evoshop.Interact_QuestWaiting(obj, activator)
	...
	UI:SetSpeakerEmotion("Worried")
	UI:WaitShowDialogue("He said they were headed to a side path deep in [color=#FFC663]Depleted Basin[color],[pause=10] past some water.[pause=0] It sounds pretty dangerous.")
	UI:SetSpeakerEmotion("Normal")
